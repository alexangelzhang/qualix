...
def __getattr__(name: str):
    _reexports = {
        "cmd_reset": "qualix.commands.phase_reset",
        "cmd_auto": "qualix.commands.phase_auto",
        "collect_inputs": "qualix.commands.phase_auto",
        "prompt_approve": "qualix.commands.phase_auto",
    }
...
def _telemetry():
    ...
    from qualix.reporting.telemetry import PhaseRunRecord, append_record, print_run_summary
...
def _flush_events() -> None:
    """Flush 事件缓冲区（静默失败）."""
    try:
        from qualix.store.events import flush_events
...
def cmd_execute(args, output_dir: Path) -> int:
    ...
    if result.success:
        print(f"\n  执行完成后运行: qualix-run {args.project_id} finalize {args.phase}")
...
def cmd_finalize(args, output_dir: Path) -> int:
    from qualix.commands.cli_json import cli_envelope, cli_json_mode, print_cli_json
    from qualix.memory.version_tracker import format_version_diff
    from qualix.quality.golden_sample import format_golden_diff
    from qualix.quality.rule_compliance import format_compliance_report
    from qualix.reporting.perf_tracker import format_metrics_report
    from qualix.runtime.execution_context import ExecutionContext
    from qualix.runtime.phase_runtime import runtime_finalize
    from qualix.skill_tracker import format_quality_report
    ...
    print(f"\n  确认通过: qualix-run {args.project_id} approve {args.phase}")
...
def cmd_approve(args, output_dir: Path) -> int:
    from qualix.commands.cli_json import cli_envelope, cli_json_mode, print_cli_json
    from qualix.quality.judge import load_judge_result
    from qualix.skill_tracker import extract_judge_cases
        ...
        if not judge_result:
            if not cli_json_mode(args):
                print(
                    ...
                    f"  命令: qualix-run {args.project_id} finalize {args.phase}",
                    file=sys.stderr,
                )
    ...
    if _judge_required and ps_pre and ps_pre.judge_score is not None and not ps_pre.judge_passed and not force:
        if not cli_json_mode(args):
            ...
            print(f"\n  使用 --force 强制通过: qualix-run {args.project_id} approve {args.phase} --force", file=sys.stderr)
    ...
    if verdict:
        ...
        if verdict.soft_blocked and not force:
            if not cli_json_mode(args):
                ...
                for c in verdict.soft_failures:
                    print(f"    [{c.source}] {c.name}: {c.message}", file=sys.stderr)
                print(
                    f"\n  使用 --force 强制通过: qualix-run {args.project_id} approve {args.phase} --force",
                    file=sys.stderr,
                )
            return _approve_out(
                False,
        ...
        from qualix.runtime.phase_contract import enforce_phase_constraints
    ...
    from qualix.core.state_machine import write_planning_snapshot
    ...
    if not cli_json_mode(args) and available:
        ...
        for group in groups:
            if len(group) > 1:
                cmds = " & ".join(f"qualix-run {args.project_id} execute {pid}" for pid in group)
                print(f"    [并行] {cmds}")
            else:
                print(f"    qualix-run {args.project_id} execute {group[0]}")
    _flush_events()
    if cli_json_mode(args):
        ps = state.phases[args.phase]
        return _approve_out(
    ...
    # bitable 上报（静默，失败不阻断）
    try:
        from qualix.telemetry.bitable import report_phase_approved
