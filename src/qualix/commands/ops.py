...
收编原 qualix-metrics / qualix-observe / qualix-regression 孤儿入口，
统一通过 qualix-run <project_id> metrics/observe/regression 调用。
"""
...
def _run_maintain(output_dir: Path) -> dict:
    ...
    from qualix.store.experiments import prune_stale_experiments
...
def cmd_observe(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    if action == "daily":
        if cli_json_mode(args):
            from qualix.reporting.observability import (
                _append_history,
                _build_alerts,
                _load_history,
                _parse_date,
        ...
        from qualix.reporting.observability import _cmd_daily
    ...
    if cli_json_mode(args):
        from qualix.reporting.observability import _parse_date, generate_report
...
def cmd_regression(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    if action == "multi-judge":
        from qualix.constants import DEFAULT_JUDGE_MODEL, DEFAULT_PRIMARY_MODEL
