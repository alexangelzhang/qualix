...
from qualix.json_utils import dump_jsonl
...
_TOOL_RESULT_RE = re.compile(r"<tool_result\s+name=\"(.*?)\">\n(.*?)\n</tool_result>", re.DOTALL)
...
def _summarize_assistant_turn(content: str) -> str:
    ...
    # 保留 tool_call 标签完整
    tool_call_match = re.search(r"<tool_call\s+name=\".*?\">.*?</tool_call>", content, re.DOTALL)
    if tool_call_match:
