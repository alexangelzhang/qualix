...
def build_requirement_graph(structured_data: dict[str, Any]) -> Any:
    ...
    for req in structured_data.get("requirements", []):
        ...
        graph.add_node(rid, type=node_type, description=req.get("description", "")[:80])

        # 边: parent → child
        parent = req.get("parent_id", "")
        if parent and parent in graph:
            graph.add_edge(parent, rid, relation="parent_child")
        elif parent:
            # parent 可能还没添加，延迟处理
            graph.add_node(parent, type="REQ", description="(forward ref)")
            graph.add_edge(parent, rid, relation="parent_child")
    ...
    for se in structured_data.get("semantic_expectations", []):
        ...
        if not sid:
            continue
        graph.add_node(sid, type="SE", description=se.get("description", "")[:80])

        # 边: SE → mapping_target (REQ/BR)
        target = se.get("mapping_target", "")
        if target and target in graph:
            graph.add_edge(sid, target, relation="maps_to")
    ...
    for gap in structured_data.get("gaps", []):
        ...
        if not gid:
            continue
        graph.add_node(gid, type="GAP", description=gap.get("description", "")[:80])
        for related in gap.get("related_ids", []):
            if related in graph:
                graph.add_edge(gid, related, relation="related_to")
    ...
    for op in structured_data.get("open_items", []):
        ...
        if not oid:
            continue
        graph.add_node(oid, type="OPEN", description=op.get("question", op.get("description", ""))[:80])
        for related in op.get("related_ids", []):
            if related in graph:
                graph.add_edge(oid, related, relation="related_to")

    return graph


def analyze_requirement_graph(graph: Any) -> RequirementGraphResult:
    """分析需求图，检测异常.
    ...
    for se in ses:
        for _, target, data in graph.out_edges(se, data=True):
    ...
    for br in brs:
        if br not in se_targets:
            desc = graph.nodes[br].get("description", "")
    ...
    for se in ses:
        out_edges = list(graph.out_edges(se, data=True))
        has_mapping = any(d.get("relation") == "maps_to" for _, _, d in out_edges)
        if not has_mapping:
            desc = graph.nodes[se].get("description", "")
            result.anomalies.append(
                GraphAnomaly(
    ...
    for req in reqs:
        children = [t for _, t, d in graph.out_edges(req, data=True) if d.get("relation") == "parent_child"]
        # 也检查入边（BR 的 parent 指向此 REQ）
        child_edges = [s for s, _, d in graph.in_edges(req, data=True) if d.get("relation") == "parent_child"]
        has_children = bool(children) or bool(child_edges)
        has_se = req in se_targets
        if not has_children and not has_se:
            desc = graph.nodes[req].get("description", "")
            result.anomalies.append(
                GraphAnomaly(
    ...
    for gap in gaps:
        out_edges = list(graph.out_edges(gap))
        if not out_edges:
            desc = graph.nodes[gap].get("description", "")
            result.anomalies.append(
                GraphAnomaly(
    ...
    for op in opens:
        out_edges = list(graph.out_edges(op))
        if not out_edges:
            desc = graph.nodes[op].get("description", "")
            result.anomalies.append(
                GraphAnomaly(
    ...
    covered_reqs = len(
        [
            req
            for req in reqs
            if req in se_targets
            or any(t for _, t, d in graph.out_edges(req, data=True) if d.get("relation") == "parent_child")
        ]
    )
...
def write_requirement_graph_analysis(
    ...
    graph = build_requirement_graph(data)
    if graph is None:
        return None

    result = analyze_requirement_graph(graph)
