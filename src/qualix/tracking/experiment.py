...
用法:
    qualix-run experiment <phase> --cycles 10 --benchmark <case_id>
"""
...
from qualix.constants import BUG_CASE_RELEVANCE_SEED_LIMIT, SKILL_FILE_MAP
...
from qualix.store import get_experiment_summary, insert_experiment, query_experiments
...
def generate_experiment_prompt(
    ...
    if benchmark_case:
        ...
        lines.append(f"1. `qualix-run {benchmark_case} execute {phase_id}`")
        lines.append(f"2. `qualix-run {benchmark_case} finalize {phase_id}`")
        lines.append(f"3. `qualix-run {benchmark_case} judge {phase_id}`")
        lines.append("4. 读取 `_judge_result.json` 获取分数")
    else:
        lines.append("使用 judge 评估改动效果：")
        lines.append(f"1. 对最近的项目执行 `qualix-run <project> judge {phase_id}`")
...
def cmd_experiment(args, output_dir: Path) -> int:
    ...
    print("\n  请用 AI IDE 读取该文件执行实验")
    print(f"  完成后: qualix-run experiment {phase_id} persist --cycle {cycle}")
    print(f"  查看日志: qualix-run experiment {phase_id} log")
    return 0
