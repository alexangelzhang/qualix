...
from qualix.json_utils import dump_json_str, dump_jsonl, load_json_strict, save_json
...
def main() -> int:
    ...
    if args.command == "prompt-eval":
        from qualix.tracking.prompt_eval import run_prompt_comparison

        return run_prompt_comparison(args.case_id, args.phase, _cases_root())

    if args.command == "rule-impact":
        from qualix.tracking.rule_impact import run_rule_impact
