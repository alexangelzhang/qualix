...
from qualix.runtime.lifecycle import register_handler
...
def handle_hard_gate(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.json_utils import load_json
    from qualix.schemas.schema_export import structured_root_model
    from qualix.text_utils import REPORT_MAP, STRUCTURED_JSON_MAP
...
def handle_perf_metrics(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.reporting.perf_tracker import collect_phase_metrics, persist_phase_metrics
...
def handle_quality_tracking(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.skill_tracker import auto_generate_bug_case, suggest_prompt_fix, track_rule_quality
...
def handle_memory_index(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.memory.sidecar_l1 import after_memory_index
...
def handle_golden_sample(ctx: ExecutionContext, result: PhaseResult) -> None:
    """Golden Sample 标杆对比."""
    from qualix.quality.golden_sample import compare_with_golden
...
def handle_rule_compliance(ctx: ExecutionContext, result: PhaseResult) -> None:
    """规则执行率追踪."""
    from qualix.quality.rule_compliance import compute_rule_compliance, persist_compliance
...
def handle_review_chain(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.quality.critique import write_critique_prompt
    from qualix.quality.judge import write_judge_prompt
    from qualix.quality.review_chain import build_review_chain_payload, write_review_chain_prompt
...
def handle_progress_file(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.runtime.progress import write_phase_progress, write_project_progress
...
def handle_skill_factory(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.tracking.skill_factory import write_skill_suggestions
    ...
    from qualix.tracking.skill_evolution import generate_evolution_report
...
def handle_auto_judge(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    """
    from qualix.quality.judge import synthesize_judge_result
...
def handle_score_calibration(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.quality.score_calibration import check_score_consistency, check_score_trend
...
def handle_eval_baseline(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.quality.eval_baseline import write_eval_metrics
...
def handle_facts_export(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.cache.fact_cache import export_facts_to_markdown
...
def handle_verification_bundle(ctx: ExecutionContext, result: PhaseResult) -> None:
    """收集所有验证结果到统一 bundle."""
    from qualix.quality.verification_bundle import write_verification_bundle
...
def handle_requirement_graph(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.quality.requirement_graph import write_requirement_graph_analysis
...
def handle_report_quality_checks(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.quality.report_quality_checks import run_report_quality_checks
...
def handle_session_failure_notes(ctx: ExecutionContext, result: PhaseResult) -> None:
    ...
    from qualix.tracking.regression import _failure_library_root
...
def handle_results_tsv(ctx: ExecutionContext, result: PhaseResult) -> None:
    """将本次 Phase 结果追加到 qualix-results.tsv（横向汇总，趋势分析用）."""
    from datetime import UTC, datetime
    ...
    tsv_path = ctx.output_dir.parent / "qualix-results.tsv"
