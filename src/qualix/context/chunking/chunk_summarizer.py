...
from qualix.constants import ID_PATTERN_BASE
    ...
    for field_key, (label, _prefix) in id_fields.items():
        ...
        for item in items[:5]:
            if isinstance(item, dict):
                for k in ("req_id", "br_id", "se_id", "gap_id", "open_id", "eut_id", "case_id", "id", "finding_id"):
                    v = item.get(k)
                    if v:
...
def summarize_upstream_chunk(chunk: ContextChunk, phase_id: str, force: bool = False) -> ContextChunk:
    ...
    """
    from qualix.context.loading.context_loader import ContextChunk as _ContextChunk
    ...
    if chunk.token_estimate > 500 and len(chunk.content) > default_char_limit:
        truncated = chunk.content[:default_char_limit] + "\n\n<!-- upstream-summarized -->"
