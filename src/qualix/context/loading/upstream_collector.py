...
from qualix.context.chunking.chunk_processor import (
    _collect_current_phase_inputs,
    _collect_phase_artifacts,
)
from qualix.context.chunking.chunk_summarizer import summarize_upstream_chunk
from qualix.context.loading.context_policy import get_context_policy
from qualix.core.model_registry import estimate_tokens
from qualix.core.profiles import get_profile, load_profile_context_l1
from qualix.core.state_machine import PHASE_DEFS, PhaseStatus
from qualix.log import get_logger
from qualix.path_utils import resolve_internal_file
from qualix.skill_tracker import render_relevant_cases_for_prompt
...
def load_upstream_context(
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        ...
        if policy.inject_gap:
            ...
            def _load_gap():
                from qualix.memory.rsm_patterns import extract_gap_patterns, format_patterns_as_checklist
        ...
        if policy.inject_rsm:
            from qualix.schemas.rsm import _rsm_path

            if _rsm_path(output_dir, project_id).exists():

                def _load_rsm():
                    from qualix.schemas.rsm import compute_coverage, load_rsm
        ...
        from .file_snapshot import diff_snapshot, load_snapshot, save_snapshot, take_snapshot
...
def load_sidecar_context(
    ...
    if mem_file.exists():
        ...
        if mem_text and mem_text.strip():
            from qualix.memory.memory_filter import filter_memory_for_phase, parse_memory_entries
    ...
    try:
        from qualix.context.skill_crystal import load_crystals_for_phase, render_crystals_for_prompt
