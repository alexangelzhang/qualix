...
from qualix.agents.agent import Agent, extract_llm_call
from qualix.agents.handoff_builder import build_handoff_document
from qualix.agents.issue_tracker import IssueTracker
from qualix.agents.judge_vote import (
    IterationRecord,
    judge_health_check,
    multi_judge_vote,
)
...
from qualix.constants import DEFAULT_ADAPTIVE_JUDGE_MODELS, STRUCTURED_JSON_MAP
...
if TYPE_CHECKING:
    ...
    from qualix.quality.evaluation_protocols import PhaseProtocol
...
class AdaptiveLoop:
    ...
    def run(
        ...
        try:
            from qualix.quality.regression.gene_store import format_genes_for_injection, load_genes_for_phase
    ...
    def _execute_iteration(
        ...
        if i == 0:
            ...
            if _force_single_phase:
                ...
                try:
                    from qualix.agents.two_phase_worker import run_two_phase_worker
        ...
        if passed and record.critique_result and record.critique_result.status != "failed":
            try:
                from qualix.quality.regression.gene_store import (
