...
全局命令（不需要 project_id）：
    ...
    qualix dashboard     — 启动/停止/状态看板
    qualix version       — 版本信息

项目命令委托给 qualix-run：
    qualix run <project_id> <command> ...
"""
...
from qualix.constants import DASHBOARD_PID_FILE as _DASHBOARD_PID_FILE
from qualix.constants import DASHBOARD_PORT as _DASHBOARD_PORT
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # Python package
    try:
        import qualix.core.runner

        print("   qualix package: OK")
    except ImportError:
        print("   qualix package: 未安装，执行 `pip install -e '.[dev]'`")
        deps_ok = False
    ...
    print("\n3. 初始化数据库...")
    try:
        from qualix.store import migrate_all
...
def _start_dashboard(port: int = _DASHBOARD_PORT) -> bool:
    ...
    try:
        from qualix.reporting import dashboard as _dashboard_mod
...
def _cmd_dashboard(args: argparse.Namespace) -> int:
    ...
    if action == "start":
        ...
        if running:
            ...
            print("看板未运行。启动: qualix dashboard start")
...
def _cmd_experiment(args: argparse.Namespace) -> int:
    """Skill 自动迭代实验."""
    from qualix.tracking.experiment import cmd_experiment

    return cmd_experiment(args, _output_dir())


def _cmd_cache(args) -> int:
    """qualix cache export <project_id> [--phase Q01] [--output path]"""
...
def main() -> int:
    ...
    # run (delegate to qualix-run)
    p_run = sub.add_parser("run", help="项目命令（等同于 qualix-run）")
    p_run.add_argument("run_args", nargs=argparse.REMAINDER, help="qualix-run 参数")
    ...
    if args.command == "run":
        # 委托给 qualix-run
        from qualix.core.runner import main as runner_main

        sys.argv = ["qualix-run", *args.run_args]
        return runner_main()
