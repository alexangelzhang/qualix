...
_TEMPLATE_PACKAGE = "qualix.reporting.templates"
...
def render_q05_eut_matrix(
    ...
    """渲染 Q05a phase_b_structured.json 为交互式 HTML kanban.

    Args:
        structured_json_path: Q05a 结构化 JSON 源路径
    ...
    if not src.exists():
        raise FileNotFoundError(f"Q05a structured JSON not found: {src}")

    payload = load_json_strict(src)

    if not isinstance(payload, dict):
        raise ValueError(f"Q05a JSON root must be object, got {type(payload).__name__}")
    if "test_cases" not in payload:
        raise ValueError("Q05a JSON missing 'test_cases' key")
