...
def split_into_sections(text: str, max_chars: int = 6000, min_section_chars: int = 100) -> list[dict[str, str]]:
    ...
    for line in lines:
        ...
        is_heading = bool(re.match(r"^#{1,4}\s+\S", line)) or bool(
            re.match(r"^\d+[\.\、]\s*[\u4e00-\u9fff]{2,}", stripped)
        )
...
def call_dashscope_text(
    ...
    try:
        ...
        raise RuntimeError("dashscope 未安装，请 pip install dashscope") from err
...
def summarize_document(
    ...
    if api_key and len(sections) > 1:
        ...
        with ThreadPoolExecutor(max_workers=effective_workers) as executor:
            future_map = {
                executor.submit(summarize_section, sec, api_key, model, timeout): i for i, sec in enumerate(sections)
            }
...
def main() -> int:
    ...
    stats = result["stats"]
    info(
        f"摘要完成: {stats['total_sections']} 章节, "
