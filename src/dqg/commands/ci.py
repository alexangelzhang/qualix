"""dqg-run <pid> ci-gate — CI/CD 质量门禁命令.

读取已有的 _gate_verdict.json，输出结构化结果并决定 exit code。
零额外 LLM 调用，速度极快（纯文件读取）。

用法：
  dqg-run mrs ci-gate Q06                    # 标准输出 + HARD block 时 exit 1
  dqg-run mrs ci-gate Q06 --fail-on soft     # SOFT 也算 fail
  dqg-run mrs ci-gate --all-phases           # 检查所有 Phase
  dqg-run mrs ci-gate Q06 --pr-comment       # 输出 GitHub PR Comment Markdown
  dqg-run mrs ci-gate Q06 --json             # JSON 输出（供 CI 脚本解析）

与 dqg-run finalize 的区别：
    ...
    if all_phases:
        ...
        if verdict is None:
            msg = (f"未找到 _gate_verdict.json（output/{args.project_id}/{phase_arg}/）。"
                   f"请先执行 dqg-run {args.project_id} finalize {phase_arg}")
            if json_mode:
                print_cli_json(cli_envelope(command="ci-gate", project_id=args.project_id,
                                            success=False, exit_code=1,
