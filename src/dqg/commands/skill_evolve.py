"""dqg-run <pid> skill-evolve — 从 failure-library 提炼 skill 改进建议.
...
典型用法：
  dqg-run mrs skill-evolve analyze
  dqg-run mrs skill-evolve suggest --phase Q03
  dqg-run mrs skill-evolve apply --phase Q03
  dqg-run mrs skill-evolve apply --phase Q03 --no-dry-run
"""
...
def _suggest(args: argparse.Namespace, output_dir: Path) -> int:
    from dqg.tracking.skill_factory import write_all_skill_suggestions, write_skill_suggestions
...
def _apply(args: argparse.Namespace, output_dir: Path) -> int:
    from dqg.tracking.skill_auto_merge import apply_to_skill_file
    from dqg.tracking.skill_evolution import generate_skill_diff
