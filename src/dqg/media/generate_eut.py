...
def parse_args():
    ...
    parser.add_argument("--input", "-i", type=str, default="ingest.json", help="Path to parsing output containing REQ/BR/SE.")
...
def generate_eut_matrix(input_path, output_path):
    ...
    if not se_list:
        se_list = [
            {"id": "SEM-001", "req": "BR-01", "desc": "订单状态机从INIT推进为PROCESSING并持久化", "verify": "status == PROCESSING"},
            {"id": "SEM-002", "req": "BR-02", "desc": "若库存扣减Ability抛出超时异常，须挂起并重试", "verify": "Throws RetryException & rollback() called"}
        ]

    md = ["# EUT (Expected Unit Test) 严控审计大纲\n"]
    md.append("> **审计警告**：以下生成的 EUT 矩阵须被单测 100% 覆盖。未来 `/ut-audit-zh` 将逐一校验下表。\n")
    md.append("| EUT编号 | 绑定语义(SEM) | 路线类型 | Given (外部装载Mock) | When (触发行为) | Then (必须满足 ut-audit-zh 的强断言约束) |")
    md.append("|---|---|---|---|---|---|")
    ...
    for se in se_list:
        ...
        # Happy
        md.append(f"| EUT-{counter:03d} | `{s_id}` | Happy Path | 正常DTO与无报错RPC拦截 | {desc} | ✅ 务必断言副产物与强状态: `{verify}` |")
        counter += 1

        # Exception - Forcing ut-audit-zh compliance
        md.append(f"| EUT-{counter:03d} | `{s_id}` | Exception | 强制 Mock 下游依赖抛出 Timeout/5xx | 异常环境进入 | 🛑 禁止含糊断言。务必断言特定错误码 `assertThrows(BizExe)` |")
        counter += 1
