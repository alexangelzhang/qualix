...
def build_demand_trace(
    ...
    # 2. 构建调用图
    from dqg.quality.blast_radius import build_call_graph_regex, get_changed_files
    ...
    log.info(
        ...
        len(entry_methods), len(trace_result["traced_methods"]),
...
def _compute_overlap(
    ...
    return {
        ...
        "recommendation": (
            "HIGH_CONFIDENCE" if overlap
            else "COMPLEMENTARY" if trace_only and blast_only
            else "TRACE_ONLY"
        ),
    }
...
def write_demand_trace(
    ...
    trace = build_demand_trace(
        output_dir, project_id, repo_path, base_branch, feature_branch,
    )
    if not trace:
        return None
...
def _render_trace_md(trace: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        f"**入口方法**: {len(trace.get('entry_methods', []))} 个"
        f"（来自 SE→Code 映射）",
        f"**追踪到**: {len(trace.get('traced_methods', []))} 个方法，"
        f"跨 {len(trace.get('traced_files', []))} 个文件",
        "",
    ]
