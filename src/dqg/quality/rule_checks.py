...
def _check_gap_level(pd: Path, report: str, phase_id: str) -> tuple[bool, str]:
    gap_lines = [l for l in report.split("\n") if RE_GAP_TABLE_LINE.match(l) or RE_GAP_DEF_LINE.match(l)]
    if not gap_lines:
        return True, "无 GAP"
    has_level = sum(1 for l in gap_lines if RE_GAP_LEVEL.search(l))
...
def _check_open_owner(pd: Path, report: str, phase_id: str) -> tuple[bool, str]:
    open_lines = [l for l in report.split("\n") if RE_OPEN_TABLE_LINE.match(l) or RE_OPEN_DEF_LINE.match(l)]
    if not open_lines:
        return True, "无 OPEN"
    has_owner = sum(1 for l in open_lines if RE_OPEN_OWNER.search(l))
...
def _check_coverage_evidence(pd: Path, report: str, phase_id: str) -> tuple[bool, str]:
    coverage_lines = [l for l in report.split("\n") if RE_COVERAGE_STATUS.search(l)]
    if not coverage_lines:
        return True, "无覆盖判定"
    has_evidence = sum(1 for l in coverage_lines if RE_COVERAGE_EVIDENCE.search(l))
