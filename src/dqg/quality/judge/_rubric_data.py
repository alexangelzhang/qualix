    ...
    "Q06": {
        ...
        "dimensions": [
            ...
            },
            {
                ...
                "description": "弱断言的测试是否被识别并处理（可通过显式 WRONG_TARGET 标记或 PARTIAL+文档说明两种方式）",
                "weight": 0.3,
                "rubric": {
                    5: "所有弱断言（assertNotNull/assertTrue(true)等）都被正确处理：或标为 WRONG_TARGET，或标为 PARTIAL 并在报告文本中明确说明弱断言问题；若报告中无弱断言则给 5 分",
                    4: "90%+ 弱断言被识别（WRONG_TARGET 或 PARTIAL+文档化），仅有 1-2 个遗漏——此分数说明弱断言识别充分，应给 PASS_WITH_CONCERNS",
                    3: "主要弱断言被检出，但超过 10% 的弱断言既未标为 WRONG_TARGET 也未以 PARTIAL 记录（隐性漏报）",
                    2: "多个明显弱断言被判为 COVERED，无论在文本还是结构化字段中均无记录",
                    1: "弱断言全部判为 COVERED，报告无任何弱断言识别记录",
                },
            },
            {
                "id": "exception_branch",
        ...
        "dimensions": [
            {
                "id": "eut_faithfulness",
                "name": "EUT 规格忠实度",
                "description": "生成的 @Test 方法是否忠实实现 Q05a EUT 矩阵中的 when/then 规格，不越权修改 EUT 定义",
                "weight": 0.35,
                "rubric": {
                    5: "每个 @Test 严格对应一条 EUT，when/then 与 EUT 规格一致，未修改 EUT 矩阵",
                    4: "90%+ @Test 忠实实现 EUT，个别方法对 then 做了合理技术细化",
                    3: "主要 @Test 符合 EUT 规格，但存在 3-5 个方法偏离 then 描述",
                    2: "超过 20% 的 @Test 与 EUT 语义不符，或存在对 EUT 矩阵的修改",
                    1: "大量 @Test 未对应 EUT 条目，或 Q05b 违规修改了 EUT 矩阵",
                },
            },
            {
                "id": "assert_strength",
                "name": "断言强度",
                "description": "生成的 @Test 是否使用强断言验证业务语义，禁止 assertTrue(true)/assertNotNull 冒充覆盖",
                "weight": 0.30,
                "rubric": {
                    5: "所有测试使用 assertEquals/assertThrows/verify(mock,times(N)) 等强断言，无空气单测",
                    4: "90%+ 测试有强断言，个别辅助方法仅做 assertNotNull 有充分理由",
                    3: "主要测试有强断言，但存在 10-20% 的弱断言或仅验证非空",
                    2: "超过 20% 的测试断言不验证业务语义，存在 assertTrue(true) 类占位符",
                    1: "大量测试无实质断言，或有明确 IRON LAW 违反（assertTrue(true) 占位）",
                },
            },
            {
            ...
            },
            {
                ...
                "rubric": {
                    ...
                    4: "基本按批次推进，passes 状态更新有 1-2 处延迟但最终一致",
                    3: "批次划分合理，但 passes 状态更新不及时或有遗漏",
                    2: "未按批次推进，大批量生成后统一编译，passes 状态混乱",
                    1: "未使用 Ralph Loop 模式，一次性生成所有测试代码",
                },
            },
        ],
    },
