...
RE_CONFIDENCE = re.compile(
    r'\[置信度[:：]\s*(High|Medium|Low)\]|置信度[:：]\s*(High|Medium|Low)|`(High|Medium|Low)`'
)
...
RE_A6_SOURCE = re.compile(
    r'\[来源[:：]|tech_design\.md|ARCH-\d+|API-\d+|DATA-\d+|EXC-\d+|PERF-\d+'
)
...
def get_rules(phase_id: str) -> list[dict[str, Any]]:
    ...
    phase_specific: dict[str, list[dict[str, Any]]] = {
        ...
        "Q02": [
            ...
            {"id": "R-INTERFACE", "name": "接口设计完整（URL+入参+响应）", "category": "质量", "check": "_check_interface_design"},
            {"id": "R-STATE-MACHINE", "name": "状态机设计（如涉及）", "category": "质量", "check": "_check_state_machine"},
        ...
        "Q05": [
            ...
            {"id": "R-HAPPY-EXCEPTION", "name": "Happy/Exception 路径均衡", "category": "质量", "check": "_check_path_balance"},
            {"id": "R-SE-BOUND", "name": "SE 绑定覆盖", "category": "质量", "check": "_check_se_bound"},
            {"id": "R-STRONG-ASSERT", "name": "使用强断言（assertEquals 非 assertNotNull）", "category": "质量", "check": "_check_strong_assert"},
        ...
        "Q06": [
            ...
            {"id": "R-C-PATH-BALANCE", "name": "②路径覆盖（Happy/Exception/Boundary均衡）", "category": "质量", "check": "_check_c_path_balance"},
            {"id": "R-C-ASSERT-STRENGTH", "name": "③断言强度（assertEquals验证业务字段）", "category": "质量", "check": "_check_c_assert_strength"},
            {"id": "R-C-MOCK-REALITY", "name": "④Mock真实性（贴近业务数据）", "category": "质量", "check": "_check_c_mock_reality"},
            {"id": "R-C-STATE-MACHINE", "name": "⑤状态机覆盖（迁移边测试）", "category": "质量", "check": "_check_c_state_machine"},
            {"id": "R-C-MAINTAINABILITY", "name": "⑥可维护性（Nested/DisplayName）", "category": "质量", "check": "_check_c_maintainability"},
            {"id": "R-C-BOUNDARY", "name": "⑦边界场景（空值/不存在/越界）", "category": "质量", "check": "_check_c_boundary"},
            {"id": "R-C-DEFENSIVE", "name": "⑧防御性测试（系统不崩溃）", "category": "质量", "check": "_check_c_defensive"},
        ],
    }
