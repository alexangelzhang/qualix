...
_COMPILED_PATTERNS: list[tuple[str, re.Pattern]] = [
    (p, re.compile(p)) for p in RATIONALIZATION_PATTERNS
]
...
_COMPILED_OVERCORRECTION: list[tuple[str, re.Pattern]] = [
    (p, re.compile(p)) for p in OVERCORRECTION_PATTERNS
]
...
def format_overcorrection_warning(result: OvercorrectionResult) -> str:
    ...
    if result.fail_without_evidence:
        parts.extend(
            f"- FAIL 缺少证据行号: {s}" for s in result.fail_without_evidence[:5]
        )
