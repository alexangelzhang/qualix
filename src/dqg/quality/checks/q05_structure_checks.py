...
# ── Q05a gate: then 字段幻觉方法名检测 ─────────────────────────────────────────
# LLM 生成 EUT 时常会在 then 字段写出不存在的方法名（如 orderService.createOrder
# 但 ExchangeOrderService 里没有 orderService 依赖）。此 gate 在 Q05a finalize
# 时扫描生产代码全集，拦截幻觉方法名进入 approved EUT 矩阵。
