...
> **Skill 豁免**：DQG 有自己的 skill 驱动执行机制。启动和执行期间跳过 superpowers skill 检查（brainstorming / writing-plans / executing-plans 等），直接按本文件和 Phase skill 执行。不要在 DQG 流程中调用任何 superpowers skill。
...
4. 执行 `dqg-run --base-dir <project_root> <project_id> startup`
5. 解析 JSON 输出，渲染菜单
6. **等待用户选择**
...
> 完整规则（11 条）见 `dqg_starter.md` 核心规则章节。
