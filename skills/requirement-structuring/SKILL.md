...
9. **枚举型 BR 组必须交叉推导** — 只看单条 BR ✗ → 同一 REQ 下多条枚举 BR 必须与同 REQ 的例外条件 BR 交叉，推导分支判定 SE ✓（典型：进度条状态路径枚举 × 非积分不记录发放状态 → 完成判定因关怀类型而异）
10. **操作型 BR 必须检查每个操作的异常路径** — "操作：A/B/C"只看正向 ✗ → 逐一检查每个操作是否有异常路径描述，无则生成 OPEN（P2）✓（典型：删除申请无校验描述 → OPEN：删除时是否触发审核校验？）
...
- [ ] 图片底部/边缘的注释文字是否遗漏？
- [ ] 正常路径和异常终结是否分开画？
- [ ] 状态机图下载失败时是否生成了 GAP P0？从 UI 截图反向提取状态机存在遗漏风险，必须标记阻断
...
### Step 2.5: ReAct 工具调用阶段（Evidence Gathering）

**目标**：在进入结构化之前，主动消解已发现的跨文档引用、不完整的状态机转换和模糊的异常路径。

**触发条件**（满足任一即进入）：
- 在 Step 2 通读中发现了跨文档引用（"详见xx文档"、"参考接口规范"等）
- 发现了状态机节点但触发条件来源不明
- 发现了异常路径描述缺乏具体的错误码或响应

**执行方式**：
对每个未解决的引用，使用以下工具之一：
- `Read`：读取被引用文档的具体片段
- `Grep`：在 evidence pack 中搜索关键词
- `AskUserQuestion`：整个 ReAct 阶段最多使用一次，保留给最高优先级的 OPEN 项

**退出条件**（满足任一即退出，立即进入 Step 3）：
- 连续 2 轮工具调用均未发现新的 SE 候选
- 已执行 5 轮工具调用（硬性上限）
- 所有已知的跨文档引用已解析

**证据记录**：
每轮工具调用结束后，将以下内容追加到 `_reasoning_log.md` 的 `## ReAct Evidence` 块：
```
### Round N
- Tool: [Read/Grep/AskUserQuestion]
- Query: [具体查询内容]
- Finding: [发现了什么，或"无新发现"]
- New SE candidates: [SE 候选列表，或"无"]
```

**注意**：
- Step 2.5 是可选的——如果 Step 2 通读后没有未解决的跨文档引用，可直接跳过进入 Step 3
- SE ID 格式必须遵循 `SE-001`（三位数字），不可用 `SE-1`
- 工具调用失败时（文件不存在等）记录为"无新发现"继续循环
...
- [ ] 存在多个入口操作同一业务实体时（如金刚位+投诉单、App+PC 双端）：是否提取了数据一致性 SE？
- [ ] 依赖外部审批/BPM/消息系统时：是否提取了该系统不可用时的降级/超时 SE 或 GAP？
...
**3b-cross. 枚举型 BR 组交叉推导（跨 BR 隐含约束）：**

单条 BR 扫描完成后，对同一 REQ 下的 BR 做以下两类交叉检查：

1. **识别枚举型 BR 组**：同一 REQ 下存在多条描述"同一对象在不同取值/状态下的行为"的 BR（如进度条在各审核状态下的路径、不同关怀类型的字段规则），将其识别为枚举组。
2. **交叉提问**：对每个枚举组，逐一提问：
   - 这组枚举是否对所有对象类型都适用？→ 有例外则生成 SE（分支判定逻辑）
   - 枚举组中是否有某个取值缺少对应 BR？→ 有则生成 GAP
   - 枚举组与同 REQ 下其他 BR 是否存在例外条件？→ 有则生成 SE（例外分支）

> **典型案例**：BR-107~114 是进度条各状态路径的枚举组，BR-039 说明非积分类型不记录发放状态。交叉后应推导出 SE：积分工单进度条完成判定为发放成功/失败，非积分工单完成判定为审核通过，两者判定条件不同。

**3b-op. 操作型 BR 完整性检查（异常路径缺失识别）：**

对每条列出了操作列表的 BR（格式如"XXX操作：A/B/C"），逐一检查每个操作是否有对应的异常路径描述：
- 有异常路径描述（前置校验/失败处理/边界条件）→ 跳过
- 无任何异常路径描述 → 生成 OPEN：该操作的前置校验逻辑是什么？决策方：研发

> **注意**：此步骤会产生较多 OPEN，需人工判断哪些是正常设计（无需特别校验）。生成 OPEN 时优先级标 P2，避免噪音干扰核心问题。

> **典型案例**：BR-104"待提交操作：删除申请/保存草稿/提交"，提交有 BR-105/106 描述异常，但删除和保存草稿无任何异常路径描述 → 应生成 OPEN：删除申请时的前置校验逻辑是什么（是否触发审核相关校验）？

**3b 即时检查：**
- [ ] 每个 SE 是否有判定依据列？→ 必须是表格格式，不是纯文字
- [ ] 状态机的每条迁移边是否都有对应 SE？→ 遍历边，逐条确认
- [ ] 循环边（如驳回→重新发起）是否有数据流定义？→ 容易遗漏
- [ ] 状态机是否列出了**非法跳转拦截** SE？合法路径有 SE ≠ 非法路径有拦截（如"待提交→发放成功"应有拦截 SE，不能只列合法路径）
- [ ] 是否识别了枚举型 BR 组并做了交叉推导？→ 有枚举组但无交叉 SE 则返工
- [ ] 操作型 BR 的每个操作是否都有异常路径描述？→ 无则生成 OPEN（P2）
