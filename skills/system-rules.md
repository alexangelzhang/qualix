...
重跑 Phase 时：
1. **读取旧版 structured JSON 作为数据基线**（确保 REQ/BR/SE/GAP/OPEN 数量不减少）
2. **报告必须按当前规范重新生成**，禁止直接复制旧版文件做增量修改。旧版可能有格式错误或内容缺陷
3. **独立判断旧版内容**：旧版产物是参考不是权威，发现旧版有错误时必须修正而非照抄
4. **写完后立即跑格式检测**（report_quality_checks），不要等 finalize 才发现格式问题
...
Phase Q04/Q03 接收技术方案时，必须按以下标准检查输入质量。**不满足最低要求时，必须阻断执行并告知用户补充。**
...
| GAP/OPEN 闭环 | Phase Q01 中每个 GAP/OPEN 的技术应对策略 |
...
**不满足时的处理**：
- 仅有 HLD 缺 LLD → 可执行 Q04（覆盖度审计），但 Q03 必须阻断，提示"技术方案缺少详细设计，无法评审实现质量"
- HLD 也不完整 → Q04 和 Q03 均阻断，提示"技术方案不满足最低输入要求"
...
所有 Phase 执行时必须遵守以下上下文大小限制：

| 场景 | 阈值 | 超出时的处理 |
|------|------|------------|
| 单个 Phase 任务的总上下文 | < 2000 行（聚焦内容） | 优先使用摘要层替代全文 |
| 上下文降级触发 | > 5000 行 | 强制触发 compact（压缩摘要），不允许全文注入 |
| 单个文件的注入量 | < 2000 行 | 截断 + 标记 `...(截断)` |
| Bug case 注入 | < 500 行（最多 8 条） | 按相关性排序取 top 8 |
| 跨项目 insights | < 300 行（最多 10 条） | 按 strength 排序取 top 10 |

> **原则**：聚焦的 2000 行优于散漫的 10000 行。Context window 大小 ≠ 注意力预算。
> 详细层级模型见 `references/context-hierarchy.md`。

## 下游→上游反馈触发机制

当下游 Phase 发现上游产物有问题时，必须显式标记而非静默处理：
...
1. 下游 Phase 在 `_reasoning_log.md` 中记录 `UPSTREAM_UPDATE_NEEDED` 标记
2. 下游 Phase 继续执行（不阻断），但在报告中标注"基于当前不完整的上游产物"
3. finalize 时输出 `UPSTREAM_UPDATE_NEEDED` 汇总
4. approve 阶段由人工决定是否需要回退更新 Phase Q01

> **原则**：下游不阻断，但问题必须可见。静默处理上游缺陷是最危险的失败模式。
