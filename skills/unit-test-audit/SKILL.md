...
IRON LAW: **Q06 是只读审计阶段。禁止创建或修改任何 `.java` / `.kt` / `.ts` 测试文件。** 输出范围仅限 `phase_c_structured.json`、`ut_audit_report.md`、`_reasoning_log.md`。覆盖率 < 80% → 在报告中标记 `FAIL`，列出 MISSING 清单，stop。不要去写测试。
...
> **覆盖率未达标 → 判 FAIL，列出 MISSING 清单，不补写测试。Q06 没有写测试的职责。**

| 指标 | 阈值 | 未达标处理 |
|------|------|----------|
| 增量行覆盖率 | **100%** | 判 FAIL，列出 MISSING 场景（公司硬性指标） |
| 增量分支覆盖率 | **100%** | 判 FAIL，列出 MISSING 场景（公司硬性指标） |
| T1 核心异常分支 | 100% 覆盖 | 列出未覆盖分支，不补写测试 |
| 变异杀伤率 T1 | >= 80% | 列出逃逸变异，不补写测试 |
| 变异杀伤率 T2 | >= 60% | 列出逃逸变异，不补写测试 |
