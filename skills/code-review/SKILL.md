...
| 前缀 | 含义 | 是否必须修复 |
|------|------|------------|
| `[Critical]` | 会导致线上故障、数据损坏、资金损失 | 必须修复，阻断合并 |
| `[Important]` | 功能缺陷、安全风险、性能问题 | 必须修复或显式 defer（附理由） |
| `[Suggestion]` | 可改进但不影响正确性 | 可选，建议采纳 |
| `[Nit]` | 代码风格、命名、格式 | 可选，不影响 approve |
| `[FYI]` | 知识分享、背景说明 | 仅供参考，无需行动 |
...
3. **评审发现** — 按 BLOCKER/CRITICAL/MAJOR/MINOR 分级，每条有 issue_id + 文件:行号 + 置信度
...
- [ ] 按 BLOCKER/MAJOR/MINOR 分级，数量汇总正确
