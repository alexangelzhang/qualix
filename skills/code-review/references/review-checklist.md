...
| # | 问题 | 不通过的处理 |
|---|------|------------|
| 1 | 现有技术栈能否解决这个问题？ | 标 `[Important]` 建议用现有方案替代 |
| 2 | 这个依赖的体积/复杂度是否合理？ | 标 `[Important]` 如果引入了重量级框架解决简单问题 |
| 3 | 是否活跃维护？（最近 6 个月有 commit） | 标 `[Suggestion]` 提醒维护风险 |
| 4 | 是否有已知安全漏洞？ | 标 `[Critical]` 如果有未修复的 CVE |
| 5 | License 是否兼容？ | 标 `[Critical]` 如果 License 不兼容 |
