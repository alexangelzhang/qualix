...
5. `CONFLICT`：有测试，断言模式与审计标准不一致，但可能是团队有意的设计决策。标记后交由人工裁决，不自动判为 WRONG_TARGET。

> **CONFLICT 使用场景**：当发现测试使用了 assertNotNull 或 verify-only 模式，但该模式在项目中是一致的、有意的（如团队约定 Gateway 层只验证调用不验证返回值），标记为 CONFLICT 而非 WRONG_TARGET。CONFLICT 不计入 FAIL 判定，但必须在报告中列出供 approve 阶段人工确认。
...
> 本步骤为 Phase Q06 的终极动态验证引擎，用于证明单测"真正在起防线作用"。
...
- 存活变异体报告传导回 Phase Q05，AI 拿着"罪证"指出具体的 EUT 漏测点和需要补强的断言方向。
