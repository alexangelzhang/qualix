...
> 轻量借鉴 OpenSpace，不引入完整自进化引擎。

## 度量维度

| 维度 | 含义 | 主要 Phase | 退化信号 |
| ---- | ---- | --------- | -------- |
| 结构化完备率 | REQ/BR 无遗漏、均有父链 | Q01 | 反复"补提 REQ" |
...
| 技术方案覆盖率 | Phase Q01 产物在技术方案中的覆盖比例 | Q04 | MISSING 占比上升 |
| 技术方案质量问题密度 | 每份方案的 WARN/FAIL 数 | Q03 | 同类问题跨方案重复 |
| Failure Mode 覆盖率 | 关键路径均有故障场景分析 | Q03 | CRITICAL_GAP 出现即阻断 |
| 变异杀伤率 | Mutation Score 达标 | Q06 | MUTATION_SURVIVED_CRITICAL 未补强 |
...
维护 1~3 份脱敏 PRD 快照，大改后跑 Phase Q01→Q06 迷你流程，对比 GAP/OPEN 数量级、WRONG_TARGET 数、ingest 成功率。目标：同一输入下质量不回退。
