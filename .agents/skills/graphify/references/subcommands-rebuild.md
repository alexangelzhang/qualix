...
Read this when invoked with `--update` or `--cluster-only`, or when any subcommand below (`query`, `path`, `explain`, `add`) starts and `.graphify_python` might be missing.
...
### Graph diff (after Step 4)

Before the merge step, back up the old graph: `cp graphify-out/graph.json graphify-out/.graphify_old.json`
...
## `--cluster-only` — re-run clustering only
