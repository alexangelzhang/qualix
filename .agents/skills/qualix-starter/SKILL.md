---
name: "qualix-starter"
description: "Start the Qualix phase workflow in Codex; use for the project quick-start flow and phase menu."
---

# Qualix Starter

Use this skill when the user asks to start the Qualix workflow, run `$qualix-starter`, or open the project phase menu.
...
按项目 starter guide 中的执行流程（步骤二~五）执行；只在用户选择 Phase 后再加载详细流程。
...
> 执行细节、sidecar 列表、finalize handler、Adaptive Loop 等均在项目 starter guide 中定义，选择 Phase 后再加载，启动时不需要。
...
> 完整规则（11 条）见项目 starter guide 核心规则章节。
