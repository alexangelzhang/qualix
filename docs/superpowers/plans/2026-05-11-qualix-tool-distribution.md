# Qualix 工具化分发 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 建立 Qualix 工具与用户项目之间的物理分发边界，让外部用户 cwd 下 `ls` 看不到 Qualix 源码，Claude 读不到就不会改。
...
| `src/qualix/commands/path_cmd.py` | Create | `qualix-run path <skills\|references\|profiles>` 只读查看内置资源 |
| `src/qualix/core/last_run.py` | Create | last-run marker 读写（`.qualix/last-run.json`） |
| `src/qualix/core/settings.py` | Create | `.qualix/settings.yaml` 加载器 + 版本一致性检查 |
| `pyproject.toml` | Modify | dynamic version、project.urls、wheel force-include |
| `src/qualix/__init__.py` | Modify | `__version__` 走 importlib.metadata |
| `src/qualix/core/profiles.py` | Modify | `_profiles_root` / `_repo_root` 迁到 ResourceResolver |
| `src/qualix/core/cli.py` | Modify | 迁移 Path(__file__) 到 ResourceResolver；不动 `_base_dir`（仍用 Path.cwd）|
| `src/qualix/core/runner.py` | Modify | startup 时记录 last-run marker；注册 init/doctor/path 子命令 |
| `src/qualix/cache/llm_result_cache.py` | Modify | Path(__file__).parent.parent 迁到 ResourceResolver |
...
Run: `cd /path/to/qualix && python -m pytest tests/test_version.py -v`
...
- [ ] **Step 5: 修改 src/qualix/__init__.py**
...
Run: `cd /path/to/qualix && pip install -e . && python -m pytest tests/test_version.py -v`
Expected: PASS

- [ ] **Step 7: Commit**

```bash
git add VERSION pyproject.toml src/qualix/__init__.py tests/test_version.py
...
**Files:**
- Create: `src/qualix/core/resource_resolver.py`
...
```python
# src/qualix/core/resource_resolver.py
...
class ResourceResolver:
    """三层回退资源查找：项目 .qualix/ -> ~/.qualix/ -> importlib.resources 兜底."""
...
```bash
git add src/qualix/core/resource_resolver.py tests/test_resource_resolver.py
...
**Files:**
- Modify: `src/qualix/core/profiles.py:41-46`
- Modify: `src/qualix/core/cli.py:174`
- Modify: `src/qualix/cache/llm_result_cache.py:97`
...
```python
# src/qualix/core/profiles.py — 替换 L41-46
from qualix.core.resource_resolver import ResourceResolver
...
```python
# src/qualix/core/cli.py L174 — 替换 Path(__file__).parents[1] / "reporting" / ...
from qualix.reporting.dashboard import __file__ as _dashboard_init
...
```python
# src/qualix/cache/llm_result_cache.py L97 — 替换
from qualix.quality.judge import __file__ as _judge_file
...
```bash
git add src/qualix/core/profiles.py src/qualix/core/cli.py src/qualix/cache/llm_result_cache.py
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = RaiseArgumentParser(
        prog="install.sh",
        description="把 Qualix 资源和 Python 包安装到 ~/.qualix + site-packages",
    )
...
def main() -> int:
    ...
    if args.dev:
        # dev 模式下 ~/.qualix/VERSION 也 symlink 回 repo，保持同步
...
Run: `./install.sh --dry-run --skip-pip --output-root /tmp/qualix-dry`
Expected: 打印安装计划包含 Qualix version、不创建 /tmp/qualix-dry
...
**Files:**
- Modify: `src/qualix/core/resource_resolver.py`（加 deprecation 检测）
...
追加到 `src/qualix/core/resource_resolver.py`：
...
class ResourceResolver:
    ...
    def check_legacy_layout(self) -> None:
        """检测 cwd 是否还是老的 Qualix repo 内布局，打印 deprecation warning."""
        legacy_signals = [
...
修改 `src/qualix/core/runner.py` 的 `main()`，在派发前调一次：

```python
# src/qualix/core/runner.py main() 内部靠近顶部
from qualix.core.resource_resolver import ResourceResolver
...
```bash
# 在 Qualix repo 根跑，应该看到 warning
python -m qualix.core.runner status 2>&1 | head -10
```
...
```bash
git add src/qualix/core/resource_resolver.py src/qualix/core/runner.py tests/test_resource_resolver.py
...
- [ ] **Step 1: 清理潜在的老 ~/.qualix**

```bash
ls ~/.qualix 2>&1 | head -5
# 如果有内容且是 dev 环境遗留，确认没有重要东西后:
# mv ~/.qualix ~/.qualix.bak.before-migration
```
...
Expected:
- `~/.qualix/skills/` 是 symlink 指向 `<repo>/skills`
- `~/.qualix/references/`、`~/.qualix/profiles/`、`~/.qualix/regression/`、`~/.qualix/VERSION` 同理
- `pip show qualix` 显示 editable install

- [ ] **Step 3: 验证 qualix-run 能读 ~/.qualix 资源**

```bash
cd /tmp
mkdir -p test-user-project && cd test-user-project
# 此时 cwd 里没有 src/qualix/
ls  # 应该只有自己放的内容
python -c "from qualix.core.resource_resolver import ResourceResolver; r = ResourceResolver(); print(r.resolve_dir('profiles'))"
```

Expected: 打印 `~/.qualix/profiles` 路径

- [ ] **Step 4: 第一批验收清单手工确认**

- [ ] `./install.sh --dry-run` 输出包含 "Qualix version" 和 "安装计划"
- [ ] `./install.sh --dev` 在 repo 根执行后，改 `skills/` 文件立即在 `~/.qualix/skills/` 可见
- [ ] `qualix-run --version` 返回 VERSION 文件内容
- [ ] 在 Qualix repo 内跑任意 qualix-run 子命令能看到 deprecation warning
- [ ] 在 /tmp/test-user-project 跑 `qualix-run` 能读到 ~/.qualix 资源不报错
...
**Files:**
- Create: `src/qualix/core/settings.py`
...
from qualix.core.settings import QualixSettings, load_settings, check_version_drift
...
```bash
git add src/qualix/core/settings.py tests/test_settings.py pyproject.toml
git commit -m "feat(distribution): .qualix/settings.yaml 加载器 + 版本一致性检查"
```
...
**Files:**
- Create: `src/qualix/commands/init.py`
- Modify: `src/qualix/core/runner.py`（注册 init 子命令）
...
from qualix.commands.init import run_init, GUARDRAIL_BEGIN, GUARDRAIL_END
...
def _settings_yaml(profile: str, qualix_version: str) -> str:
    return (
        f"# Qualix 项目配置 — 由 qualix-run init 生成\n"
...
def run_init(project_root: Path, profile: str, force: bool) -> int:
    ...
    if qualix_root.exists() and not force:
        print(f"错误: {qualix_root} 已存在。使用 --force 覆盖（会删除原有 .qualix/ 及其 code_repos 配置）")
        return 1
    if qualix_root.exists() and force:
        print(f"⚠️  --force 模式将删除 {qualix_root}")
        shutil.rmtree(qualix_root)

    (qualix_root / "output").mkdir(parents=True)
    try:
        qualix_version = _version("qualix")
    except Exception:
        qualix_version = "unknown"
    (qualix_root / "settings.yaml").write_text(_settings_yaml(profile, qualix_version))
...
修改 `src/qualix/core/runner.py` 的 `_build_parser()`，在现有子命令后追加：

```python
init_p = subparsers.add_parser("init", help="在当前目录初始化 .qualix/ 工作区")
init_p.add_argument("--profile", default="java-ddd", help="默认 profile")
init_p.add_argument("--force", action="store_true", help="删除已有 .qualix/ 重建")
```
...
```bash
git add src/qualix/commands/init.py src/qualix/core/runner.py tests/test_init_cmd.py
git commit -m "feat(distribution): qualix-run init 建 .qualix/ 工作区 + 注入 CLAUDE.md guardrail"
```
...
**Files:**
- Create: `src/qualix/commands/path_cmd.py`
- Modify: `src/qualix/core/runner.py`（注册 path 子命令）
...
```python
# src/qualix/commands/path_cmd.py
from __future__ import annotations
...
```bash
git add src/qualix/commands/path_cmd.py src/qualix/core/runner.py tests/test_path_cmd.py
...
**Files:**
- Modify: `src/qualix/core/runner.py`（main 入口检测 drift）
...
def test_drift_warning_prints(tmp_path, monkeypatch):
    ...
    # 非零退出码也行（.qualix 未完整），但 stderr 应该出现 drift warning
...
```python
# src/qualix/core/runner.py main() 靠近 check_legacy_layout() 的地方追加
...
- [ ] `qualix-run init` 在空项目建出 `.qualix/output/` + settings.yaml + 注入 CLAUDE.md
- [ ] 重跑 `qualix-run init --force` CLAUDE.md guardrail 只有一份
- [ ] `qualix-run path skills` 打印 `~/.qualix/skills` 绝对路径
- [ ] 用户项目里改 settings.yaml 的 qualix_version 后跑 qualix-run 看到漂移 warning

- [ ] **Step 6: Commit**

```bash
git add src/qualix/core/runner.py tests/test_version_drift_warning.py
...
**Files:**
- Create: `src/qualix/core/last_run.py`
- Modify: `src/qualix/core/runner.py`
...
from qualix.core.last_run import write_last_run, read_last_run
...
_LAST_RUN = ".qualix/last-run.json"
...
修改 `src/qualix/core/runner.py` 的 `main()`，在 return exit_code 前捕获：
...
```bash
git add src/qualix/core/last_run.py src/qualix/core/runner.py tests/test_last_run.py
...
**Files:**
- Create: `src/qualix/commands/doctor.py`
...
def test_version_consistency_mismatch_flag(project_with_state):
    ...
    (Path.home() / ".qualix").mkdir(exist_ok=True)
...
def _collect_env() -> str:
    lines = []
    try:
        lines.append(f"qualix version: {_version('qualix')}")
    except Exception as e:
        lines.append(f"qualix version: unknown ({e})")
...
def check_version_consistency(
    ...
    if settings_path.exists():
        for line in settings_path.read_text().splitlines():
            if line.strip().startswith("qualix_version:"):
...
def build_bundle(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # settings.yaml
        settings = project_root / ".qualix" / "settings.yaml"
        if settings.exists():
        ...
        # input-summary
        lines = []
        for proj in (project_root / ".qualix" / "output").glob("*"):
            for f in proj.rglob("*"):
                if f.is_file():
...
```bash
git add src/qualix/commands/doctor.py tests/test_doctor_cmd.py
...
**Files:**
- Modify: `src/qualix/commands/doctor.py`（加 upload 函数和主入口）
- Modify: `src/qualix/core/runner.py`（注册 doctor 子命令）
...
def test_parse_issue_url():
    out = "creating issue...\nhttps://github.com/alexangelzhang/qualix/-/issues/42\n"
    assert parse_issue_url_from_stdout(out) == "https://github.com/alexangelzhang/qualix/-/issues/42"
...
def test_upload_via_glab_success(mock_run, tmp_path):
    ...
    mock_run.return_value = MagicMock(
        returncode=0,
        stdout="created: https://github.com/alexangelzhang/qualix/-/issues/7\n",
        stderr="",
    )
...
```python
# src/qualix/commands/doctor.py 追加
...
```bash
git add src/qualix/commands/doctor.py src/qualix/core/runner.py tests/test_doctor_upload.py
...
- [ ] `./install.sh --dev` 改 skills/ 文件立即在 `~/.qualix/skills/` 可见
