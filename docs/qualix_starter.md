---
name: qualix-execution-guide
description: "Qualix Phase 执行指南 — 用户选择 Phase 后按需加载"
---

# Qualix Phase 执行指南

> 启动逻辑见 `$qualix-starter` skill 或对应 AI IDE 入口。
...
SubAgent prompt 模板：
```
你是 Qualix Phase {phase_id} 的执行 Agent。
...
**手动模式（用户通过 Qualix starter 菜单交互）：一律直接执行，不派 SubAgent。** 主会话直接读取 skill 文件执行，可以实时验证每一步产出。SubAgent 有独立 context，产出质量不可控。
