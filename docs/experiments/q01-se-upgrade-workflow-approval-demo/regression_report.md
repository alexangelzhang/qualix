# Q01 SE Checklist 升级 — workflow-approval-demo 影子回归报告

> 实验日期：2026-05-10
> 输入：已 approved 的 18 条 SE + workflow-approval-demo PRD（147 行）+ 图片语义（21 张）+ 新版 checklist prompt（含示例对照）
...
4. **建立 regression：本次实验的 18 + 14 = 32 条 SE 可作为 workflow-approval-demo 的 golden set**
   后续任何 Q01 prompt 改动，都在 workflow-approval-demo 跑一次，比对生成的 SE 集合与 golden set 的召回率（是否漏了本报告中的 14 条新增点）和描述强度（STRONG 比例是否提升到 70%+）。当前基线：STRONG 比例 16.7%，目标：50%+。
