...
Q05a Phase 专属的 PhaseGuardrail。校验 EUT 对业务方法各分支（happy / 边界 / 异常 / 并发）的覆盖率，关键方法（含 throws / try-catch）必须 ≥1 条异常分支 EUT，否则 BLOCKED。配合 Q05a EUT 矩阵设计三步范式（分支枚举 → 业务后果映射 → 断言对准）使用。
- _Avoid_: 与通用 PhaseGuardrail 混用（这是 Q05a 专属挂载，只在 `get_phase_guardrails("Q05a")` 时生效）
- Relationships: 由 T6 Q05a 范式改造引入；Q05a 已拆为 Q05a（EUT 设计）+ Q05b（代码生成），本 Guardrail 属于 Q05a 层
