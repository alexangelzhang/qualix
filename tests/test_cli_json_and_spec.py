...
def test_envelope_always_has_schema_version() -> None:
    from qualix.commands.cli_json import CLI_ENVELOPE_SCHEMA_VERSION, cli_envelope
...
def test_envelope_errors_only_when_provided() -> None:
    from qualix.commands.cli_json import cli_envelope
...
def test_build_phase_contract_does_not_write(tmp_path: Path) -> None:
    from qualix.runtime.phase_contract_builder import build_phase_contract
...
def test_json_schema_uses_serialization_mode() -> None:
    from qualix.schemas.schema_export import json_schema_for_phase
...
def test_resolve_output_dir_quiet_suppresses_stderr(tmp_path: Path, capsys) -> None:
    ...
    from qualix.core.runner import _resolve_output_dir
...
def test_cmd_spec_missing_phase_returns_envelope_errors(tmp_path: Path) -> None:
    from qualix.commands.cli_json import CLI_ENVELOPE_SCHEMA_VERSION
