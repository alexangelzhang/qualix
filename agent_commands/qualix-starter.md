...
读取 `docs/qualix_starter.md` 中的执行流程（步骤二~五），然后按流程执行：
...
> 执行细节、sidecar 列表、finalize handler、Adaptive Loop 等均在 `docs/qualix_starter.md` 中定义，选择 Phase 后再加载，启动时不需要。
...
> 完整规则（11 条）见 `docs/qualix_starter.md` 核心规则章节。
